...
Full-stack traffic monitoring web application với real-time data visualization, AI-powered agents, và comprehensive analytics dashboard. Application này query data từ shared infrastructure (Stellio, Neo4j, Fuseki, PostgreSQL) được generate bởi Python orchestrator.
...
│  ├── REST API (12 route groups)                     │
│  ├── WebSocket Server (real-time broadcasting)      │
│  ├── 3 AI Agents (GraphRAG, EcoTwin, TrafficMaestro)│
│  └── Data Services (Stellio, Neo4j, Fuseki, Postgres)│
│                                                      │
└─────────────────────────────────────────────────────┘
         ↓ Query Data From ↓
┌─────────────────────────────────────────────────────┐
...
│  ├── Neo4j (accident graph)                         │
│  ├── Fuseki (RDF triples)                           │
│  └── PostgreSQL (metrics & predictions)             │
└─────────────────────────────────────────────────────┘
         ↑ Generated By ↑
┌─────────────────────────────────────────────────────┐
│              Python Orchestrator (UIP)               │
...
- **Notification System**: Custom toast notifications với sound alerts
...
**Use Case**: When accident detected, investigate với full context để xác định:
...
**Use Case**: User asks "Hôm nay có nên đi xe máy không?" → Agent provides:
...
**Purpose**: Predictive event orchestrator correlating traffic patterns với external events.
...
**Use Case**: Concert at Saigon Opera House (5000 attendees) → Agent:
...
│   ├── config/
│   │   ├── entities.yaml          # NGSI-LD entity mappings
...
├── start-dev.ps1                  # Dev launcher script
└── README.md                      # This file
```
...
Domain-agnostic NGSI-LD entity definitions với automatic field mapping:
...
```
Python Orchestrator → Stellio/Neo4j/Fuseki/Postgres
                              ↓
...
This component is developed by the UIP Team:
...
**Parent Project**: UIP - Urban Intelligence Platform  
