...
class AcceptHeaderParser:
    ...
    def parse(accept_header: str) -> List[AcceptFormat]:
        ...
        formats.sort(key=lambda f: (f.quality, f.mime_type != "*/*", "/" in f.mime_type), reverse=True)
    ...
    def render(
        ...
        try:
            ...
            context = {
                ...
                'base_url': base_url,
                'formats': [
                    {'name': 'JSON-LD', 'param': 'jsonld', 'mime': 'application/ld+json'},
                    {'name': 'Turtle', 'param': 'turtle', 'mime': 'text/turtle'},
                    {'name': 'RDF/XML', 'param': 'rdfxml', 'mime': 'application/rdf+xml'}
                ]
            }
...
class ContentNegotiationAgent:
    ...
    def negotiate_format(
        self,
        accept_header: str,
        available_formats: Optional[List[FormatConfig]] = None
    ) -> NegotiationResult:
        """
