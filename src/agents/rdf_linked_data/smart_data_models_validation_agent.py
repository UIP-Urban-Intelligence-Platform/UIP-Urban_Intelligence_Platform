...
class SmartDataModelsValidationAgent:
    ...
    def __init__(self, config_path: str = "config/validation.yaml"):
        ...
        self.lod_calculator = LODRatingCalculator(self.config.get("lod_criteria", {}))
        self.validator = NGSILDValidator(self.config.get("validation", {}))
        self.report_generator = ValidationReportGenerator(self.config.get("report", {}))
    ...
    def validate_entity(
        ...
        if not is_valid:
            self.logger.warning(f"Entity {entity_id} validation failed: {errors}")
        else:
            self.logger.debug(f"Entity {entity_id} validated: {lod_rating} stars")
...
def main(config: Dict = None):
    ...
    parser.add_argument("--source", help="Source entities file (overrides config)")
