                ...
                if type_uri:
                    ...
                    graph.add(
                        (
                            subject,
                            RDF.type,
                            URIRef(f"https://uri.etsi.org/ngsi-ld/{entity_type}"),
                        )
                    )
            else:
                graph.add(
...
class RDFSerializer:
    ...
    def _serialize_to_file(
        ...
        with open(output_path, "wb") as f:
            graph.serialize(destination=f, format=format_name, encoding="utf-8")
...
class NGSILDToRDFAgent:
    ...
    def convert(
        ...
        try:
            ...
            self.statistics.errors.append({"stage": "parsing", "error": str(e)})
            return self.statistics
        ...
        try:
            ...
            self.statistics.errors.append({"stage": "serialization", "error": str(e)})
