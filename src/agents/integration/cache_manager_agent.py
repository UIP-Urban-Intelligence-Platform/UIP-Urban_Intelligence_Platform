...
class CacheKeyGenerator:
    ...
    @staticmethod
    def generate(url: str, params: Optional[Dict[str, Any]] = None, 
                 headers: Optional[Dict[str, str]] = None,
                 body: Optional[bytes] = None,
                 vary_by: Optional[List[str]] = None) -> str:
        """
...
class CacheInvalidator:
    ...
    async def invalidate_by_event(self, event: str, entity_id: Optional[str] = None,
        ...
        event_config = next(
            (e for e in webhook_strategy.get('events', []) if e['event'] == event),
            None
        )
...
class CacheManagerAgent:
    ...
    def __init__(self, config_path: str):
        ...
        self.default_policy = next(
            (p for p in self.policies if p.name == 'default'),
            None
        )
    ...
    async def set(self, cache_key: str, value: bytes, ttl: int,
                  compress: bool = True, compress_min_size: int = 1024) -> bool:
        """
        Set value in cache
        ...
        try:
            ...
            return {
                ...
                'components': {
                    ...
                    'warmer': {
                        'status': 'enabled' if self.warmer and self.warmer.enabled else 'disabled'
                    },
                    'invalidator': {
                        'status': 'enabled' if self.invalidator and self.invalidator.enabled else 'disabled'
                    }
                },
...
try:
    ...
    def create_app(config_path: str = "config/cache_config.yaml") -> FastAPI:
        ...
        async def invalidate_webhook(
            ...
            webhook_strategy = next(
                (s for s in invalidation_config.get('strategies', [])
                 if s['type'] == 'webhook'),
                None
            )
