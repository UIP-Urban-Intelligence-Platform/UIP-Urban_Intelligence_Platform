...
class CircuitBreaker:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.backend_overrides = {bc["backend"]: bc for bc in backend_configs}
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if api_key:
            ...
            allowed, rate_limit_info = await self.rate_limiter.check_rate_limit(
                api_key.key, api_key.rate_limit, request.method, request.url.path
            )
            ...
            allowed, rate_limit_info = await self.rate_limiter.check_rate_limit(
                f"ip:{client_ip}", None, request.method, request.url.path
            )
        ...
        for attempt in range(max_attempts):
            try:
                ...
                if attempt < max_attempts - 1:
                    ...
                    self.logger.warning(
                        f"Request failed (attempt {attempt + 1}/{max_attempts}), retrying in {wait_time}s: {e}"
                    )
