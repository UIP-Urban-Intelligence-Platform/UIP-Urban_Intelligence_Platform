...
class Neo4jConnector:
    ...
    def create_relationship(self, tx: Transaction, from_id: str, to_id: str, 
...
class Neo4jSyncAgent:
    ...
    def _sync_item_flow_observed(self, entity: Dict[str, Any], camera_mapping: Dict[int, str]) -> None:
        ...
        def work(tx: Transaction, entity: Dict[str, Any], camera_mapping: Dict[int, str]):
            ...
            for key in ['vehicleCount', 'averageSpeed', 'intensity', 'occupancy', 'congestionLevel', 'observedAt']:
                ...
                if "Camera:" in ref_device:
                    ...
                    if camera_index_str.isdigit():
                        ...
                        if camera_index in camera_mapping:
                            ...
                            logger.debug(f"Skipping observation {entity_id}: Camera index {camera_index} not in mapping (max: {len(camera_mapping)-1})")
                ...
                for key in possible_keys:
                    if key in payload:
                        ...
                        if isinstance(prop_data, dict):
                            ...
                            has_value = prop_data.get('https://uri.etsi.org/ngsi-ld/hasValue', [])
                            if has_value:
            ...
            for loc_key in location_keys:
                if loc_key in payload:
                    ...
                    if isinstance(loc_data, dict):
                        has_value = loc_data.get('https://uri.etsi.org/ngsi-ld/hasValue', [])
                        if has_value and isinstance(has_value, list) and len(has_value) > 0:
                            wkt_val = has_value[0]
                            ...
                            if isinstance(wkt_val, str) and wkt_val.startswith('POINT'):
                                try:
                                    ...
                                    coords_str = wkt_val.replace('POINT', '').strip().strip('()')
                        ...
                        # Also try simple format
                            ...
                            if isinstance(loc_value, dict) and loc_value.get('type') == 'Point':
            ...
            for db_key in detected_by_keys:
                if db_key in payload:
                    ...
                    if isinstance(db_data, dict):
                        # JSON-LD expanded format
                        has_object = db_data.get('https://uri.etsi.org/ngsi-ld/hasObject', [])
                        if has_object and isinstance(has_object, list) and len(has_object) > 0:
                            camera_obj = has_object[0]
                            camera_id = camera_obj.get('@id') if isinstance(camera_obj, dict) else camera_obj
                        else:
                            # Simple format
                        ...
                        if camera_id:
                            try:
                                ...
                                """
                                rel_result = tx.run(rel_cypher, accident_id=entity_id, camera_id=camera_id)
                                rel_record = rel_result.single()
                                if rel_record:
                                    logger.debug(f"Created HAS_ACCIDENT: {camera_id} -> {entity_id}")
                            except Exception as e:
                                logger.debug(f"Could not create HAS_ACCIDENT for {entity_id}: {e}")
                    break
...
def main(config: Optional[Dict[str, Any]] = None):
    ...
    if config:
        ...
        parser.add_argument('--config', default='config/neo4j_sync.yaml',
