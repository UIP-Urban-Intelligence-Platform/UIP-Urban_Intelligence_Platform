        ...
        if self.enabled and REDIS_TIMESERIES_AVAILABLE:
            try:
                ...
                logger.info(
                    ...
                    extra={"redis": f"{redis_host}:{redis_port}", "retention_days": self._retention_days}
                )
...
def create_temporal_tracker(config: Optional[Dict[str, Any]] = None) -> TemporalStateTrackerAgent:
