        ...
        for name, config in self.transforms.items():
            ...
            if transform_type == 'boolean_map':
                self._transform_functions[name] = self._create_boolean_map(
                    config.get('true_value'),
                    config.get('false_value')
                )
...
class NGSILDTransformerAgent:
    ...
    def apply_property_mapping(self, entity: Dict[str, Any], 
                               source_field: str, 
                               mapping_config: Any) -> Optional[Dict[str, Any]]:
        """
    ...
    def create_weather_observed_entity(self, camera_entity: Dict[str, Any], 
    ...
    def create_air_quality_observed_entity(self, camera_entity: Dict[str, Any],
                                          camera_id: str) -> Optional[Dict[str, Any]]:
        """
                ...
                if self.config['processing'].get('validate_output', True):
                    if self.validator.validate_entity(camera_entity):
                        ...
                        if weather_entity_id:
                            camera_entity['refWeatherObserved'] = self.create_relationship(weather_entity_id)
                            self.logger.debug(f"Added refWeatherObserved relationship to {camera_id}")
                        
                        if aq_entity_id:
                            camera_entity['refAirQualityObserved'] = self.create_relationship(aq_entity_id)
                            self.logger.debug(f"Added refAirQualityObserved relationship to {camera_id}")
    ...
    def save_output(self, ngsi_entities: List[Dict[str, Any]], 
...
def main(config: Dict = None):
    ...
    parser.add_argument(
        '--source',
        help='Path to source JSON file (overrides config)'
    )
    parser.add_argument(
        '--output',
