...
class BusinessRulesEngine:
    ...
    def _create_evaluation_context(
        ...
        if isinstance(field_value, (int, float)):
            ...
            # For string values
            for name in ['id', 'url', 'observedAt', 'dateObserved', 'description', 'imageSnapshot', 'streamURL', 'status', 'patientId']:
                context[name] = field_value
...
class DataQualityValidatorAgent:
    ...
    def _log_validation_result(self, report: Dict[str, Any]):
        ...
        log_level = self.reporting_config.get('log_level', 'INFO')
        
        message = (
            f"Entity {entity_id}: {status} "
...
def main():
    ...
    parser.add_argument(
        '--entity',
        help='Path to entity JSON file to validate'
    )
    parser.add_argument(
        '--batch',
        help='Path to JSON file containing array of entities'
    )
