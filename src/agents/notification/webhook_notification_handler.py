...
class WebhookNotificationHandler:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if self.enabled and REQUESTS_AVAILABLE:
            ...
            logger.info(
                ...
                extra={"endpoints": len(self._endpoints), "max_retries": self._max_retries}
            )
        else:
            logger.info(
                "WebhookNotificationHandler in SAFE MODE (disabled)",
                extra={"enabled": self.enabled, "requests_available": REQUESTS_AVAILABLE}
            )
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        open_circuits = [
            url for url, failures in self._circuit_breakers.items()
