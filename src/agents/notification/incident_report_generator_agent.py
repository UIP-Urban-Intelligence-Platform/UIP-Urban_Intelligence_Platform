...
class ReportDataCollector:
    ...
    def collect_incident_data(self, accident_id: str, entity_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if camera_id:
            ...
            timeline_data = self.neo4j.execute_query('timeline', {
                'camera_id': camera_id,
        ...
        if location and isinstance(location, dict):
            ...
            if len(coordinates) >= 2:
                ...
                related_data = self.neo4j.execute_query('related_incidents', {
                    'accident_id': accident_id,
                    'radius_meters': 1000,
                    'start_time': start_time,
                    'end_time': end_time
        ...
        if location and isinstance(location, dict):
            ...
            if len(coordinates) >= 2:
                ...
                historical_data = self.neo4j.execute_query('historical_patterns', {
        ...
        # Get weather context
        weather_data = self.neo4j.execute_query('weather_context', {
...
class VisualizationGenerator:
    ...
    def generate_speed_timeline_chart(self, timeline_data: List[Dict[str, Any]]) -> Optional[str]:
        ...
        try:
            ...
            plt.plot(range(len(speeds)), speeds, marker='o', color='#FF6B6B', linewidth=2, markersize=6)
    ...
    def generate_speed_variance_chart(self, timeline_data: List[Dict[str, Any]]) -> Optional[str]:
        ...
        try:
            ...
            # Create chart
            plt.figure(figsize=(10, 6))
            plt.bar(range(len(variances)), variances, color='#95E1D3', edgecolor='#5FBE9B', linewidth=1.5)
