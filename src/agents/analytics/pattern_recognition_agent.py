...
class Neo4jConnector:
    ...
    def is_ready_for_pattern_analysis(self) -> Tuple[bool, str]:
        ...
        if not self.check_observation_nodes_exist():
            return (
                False,
                "Observation nodes not found - Neo4j sync may not have completed yet",
            )
        ...
        if not self.check_has_observation_relationship_exists():
            return (
                False,
                "HAS_OBSERVATION relationships not found - data sync incomplete",
            )
...
class PatternDetector:
    ...
    def detect_anomalies(
        self, metric: str = "intensity", threshold: float = 2.5, min_samples: int = 30
    ) -> List[Dict[str, Any]]:
        """
...
class PatternRecognitionAgent:
    ...
    def create_pattern_entity(
        ...
        if entity_config.get("relationships", {}).get("link_to_camera", True):
            entity["refCamera"] = {"type": "Relationship", "object": camera_id}
    ...
    def process_all_cameras(self, time_window: str = "7_days") -> Dict[str, Any]:
        ...
        for camera_id in cameras:
            try:
                ...
                if not analysis or analysis.get("status") in [
                    "skipped",
                    "failed",
                    "no_data",
                ]:
