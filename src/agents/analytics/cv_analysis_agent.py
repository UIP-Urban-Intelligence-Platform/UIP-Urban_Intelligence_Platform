...
class ImageDownloader:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.user_agent = conn_config.get('user_agent', 
...
class NGSILDEntityGenerator:
    ...
    def create_accident_entity(
        ...
        """
        # Generate unique ID
        ts_clean = timestamp.replace(':', '').replace('-', '').replace('.', '').replace('Z', '')
...
class CVAnalysisAgent:
    ...
    def analyze_image(self, camera_id: str, image: Image.Image, image_url: str = "") -> ImageAnalysisResult:
        ...
        try:
            ...
            if accident_detections:
                ...
                logger.info(f"📝 Set metadata.accidents for camera {camera_id}: {len(result.metadata['accidents'])} accidents")
    ...
    async def process_cameras(self, cameras: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
            ...
            for camera in batch:
                ...
                if result.status == DetectionStatus.SUCCESS or result.status == DetectionStatus.NO_DETECTIONS:
                    ...
                    entity = NGSILDEntityGenerator.create_item_flow_observed(
                        ...
                        detections=result.detections if self.config_loader.get_output_config().get('include_detections') else None
                    )
                    ...
                    if result.metadata:
                        logger.debug(f"Camera {camera_id} metadata keys: {list(result.metadata.keys())}")
                    
                    if result.metadata and 'accidents' in result.metadata:
                        accidents = result.metadata['accidents']
                        for idx, acc in enumerate(accidents):
                            accident_entity = NGSILDEntityGenerator.create_accident_entity(
                                camera_id=camera_id,
                                location=location,
        ...
        if self._detected_accidents:
            logger.info(f"📊 Total accidents detected: {len(self._detected_accidents)} across all cameras")
    ...
    def save_accidents(self, output_file: str = 'data/accidents.json') -> None:
        ...
        if self._mongodb_helper and self._mongodb_helper.enabled and accidents:
            try:
                ...
                if failed > 0:
                    logger.warning(f"⚠️ Failed to publish {failed} accident entities to MongoDB")
            except Exception as e:
                logger.warning(f"MongoDB publishing failed (non-critical): {e}")
