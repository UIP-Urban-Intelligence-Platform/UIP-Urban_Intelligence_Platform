    ...
    def detect(
        ...
        if len(observations) < self.window_size:
            return (
                False,
                0.0,
                f"insufficient_data: {len(observations)}/{self.window_size}",
            )
    ...
    def detect(
        ...
        if len(observations) < self.baseline_window + 1:
            return (
                False,
                0.0,
                f"insufficient_data: {len(observations)}/{self.baseline_window + 1}",
            )
...
class AccidentDetectionAgent:
    ...
    def _build_accident_entity(
        ...
        entity = {
            ...
            "detectionMethod": {"type": "Property", "value": ", ".join(methods)},
        }

        # Add location if available
        if location_value:
            entity["location"] = {"type": "GeoProperty", "value": location_value}

        # Add camera relationship
        if self.entity_cfg.get("link_to_camera", True):
            entity["refCamera"] = {"type": "Relationship", "object": camera_ref}

        # Add observation reference
        if self.entity_cfg.get("link_to_observations", True):
            obs_id = observation.get("id")
            if obs_id:
                entity["refObservation"] = {"type": "Relationship", "object": obs_id}
...
def main(config: Optional[Dict[str, Any]] = None):
    ...
    if config:
        ...
        parser.add_argument(
            "--config",
            default="config/accident_config.yaml",
            help="Path to accident config",
        )
