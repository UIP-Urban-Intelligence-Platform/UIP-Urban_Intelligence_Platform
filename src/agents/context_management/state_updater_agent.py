...
class WebhookEventSource:
    ...
    def start(self) -> None:
        ...
        def health():
            return (
                jsonify({"status": "healthy", "queue_size": self.event_queue.qsize()}),
                200,
            )
    ...
    def patch_entity(self, event: UpdateEvent, retry_count: int = 0) -> UpdateResult:
        ...
        try:
            ...
            if (
                ...
                logger.warning(
                    f"Retrying update for {event.entity_id} (attempt {retry_count + 1}/{self.max_retry_attempts}) after {delay:.2f}s"
                )
                time.sleep(delay)
        ...
        try:
            while self.running:
                ...
                if events:
                    ...
                    logger.info(
                        f"Processed {len(results)} updates: {success_count} succeeded, {len(results) - success_count} failed"
                    )
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/state_updater_config.yaml",
