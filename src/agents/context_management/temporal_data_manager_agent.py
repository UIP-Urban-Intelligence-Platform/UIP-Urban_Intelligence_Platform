...
class RetentionManager:
    ...
    def __init__(self, config: TemporalConfig):
        ...
        logger.info(
            f"Retention policies: detailed={self.detailed_days}d, aggregated={self.aggregated_days}d, archived={self.archived_days}d"
        )
...
class TemporalDataManagerAgent:
    ...
    def run_cleanup(
        ...
        """
        results = {"aggregated": 0, "archived": 0, "deleted": 0}
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="config/temporal_config.yaml",
