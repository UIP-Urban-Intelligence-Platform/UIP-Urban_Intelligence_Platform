...
class ExternalDataCollectorAgent:
    ...
    async def _fetch_weather_data_cached(self, latitude: float, longitude: float,
    ...
    async def fetch_weather_data(self, session: aiohttp.ClientSession,
        ...
        result = await self.retry_handler.execute(
            self._fetch_weather_data_cached,
            latitude,
            longitude,
            session_id
        )
    ...
    @alru_cache(maxsize=128, ttl=600)  # Cache for 10 minutes (MANDATORY requirement)
    async def _fetch_air_quality_data_cached(self, latitude: float, longitude: float,
                                             session_id: int) -> Optional[Dict[str, Any]]:
        """
    ...
    async def fetch_air_quality_data(self, session: aiohttp.ClientSession,
                                     latitude: float, longitude: float) -> Optional[Dict[str, Any]]:
        """
        ...
        # Use retry handler with exponential backoff (MANDATORY: 3 attempts)
        result = await self.retry_handler.execute(
            self._fetch_air_quality_data_cached,
            latitude,
            longitude,
            session_id
        )
    ...
    async def enrich_entity(self, session: aiohttp.ClientSession,
