...
def create_complex_entity():
    ...
    return {
        ...
        "refSimple": {
            "type": "Relationship",
            "object": "urn:ngsi-ld:Related:REL001"
        },
...
def compare_entities(original, retrieved, path=""):
    ...
    if type(original) != type(retrieved):
        issues.append(f"{path}: Type mismatch - original={type(original).__name__}, retrieved={type(retrieved).__name__}")
        return issues
    
    if isinstance(original, dict):
        # Check all keys exist
        original_keys = set(original.keys())
        retrieved_keys = set(retrieved.keys()) - {'_id', '_insertedAt', '_updatedAt'}  # Exclude metadata
        ...
        if len(original) != len(retrieved):
            issues.append(f"{path}: Array length mismatch - original={len(original)}, retrieved={len(retrieved)}")
        else:
        ...
        if original != retrieved:
            issues.append(f"{path}: Value mismatch - original={original}, retrieved={retrieved}")
...
def test_deep_preservation():
    ...
    print(f"  - Properties: {len([k for k, v in original_entity.items() if isinstance(v, dict) and v.get('type') == 'Property'])}")
    print(f"  - GeoProperties: {len([k for k, v in original_entity.items() if isinstance(v, dict) and v.get('type') == 'GeoProperty'])}")
    print(f"  - Relationships: {len([k for k, v in original_entity.items() if isinstance(v, dict) and v.get('type') == 'Relationship'])}")
    print(f"  - Total attributes: {len([k for k in original_entity.keys() if k not in ['id', 'type', '@context']])}")
    ...
    retrieved = helper.find_entity(original_copy['id'], entity_type=original_copy['type'])
    ...
    for key, value in original_copy.items():
        if isinstance(value, dict) and value.get('type') == 'Property':
            ...
            print(f"    {status} {key}: type={value.get('type')}, value preserved={match}")
    ...
    for key, value in original_copy.items():
        if isinstance(value, dict) and value.get('type') == 'GeoProperty':
            ...
            print(f"    {status} {key}: type={value.get('type')}, GeoJSON={geo_type}, preserved={match}")
    ...
    for key, value in original_copy.items():
        if isinstance(value, dict) and value.get('type') == 'Relationship':
            match = retrieved.get(key) == value
            status = "✅" if match else "❌"
            print(f"    {status} {key}: type={value.get('type')}, object={value.get('object')}, preserved={match}")
