...
def verify_stellio_final_state(entity_id):
    ...
    if response.status_code == 200:
        ...
        if ai_metadata:
            ...
            print(
                f"   - Avg confidence: {ai_metadata.get('avg_detection_confidence', 'N/A')}"
            )

            if "accident_detected" in ai_metadata:
                print(f"   - Accident detected: {ai_metadata['accident_detected']}")
                print(
                    f"   - Accident confidence: {ai_metadata.get('accident_confidence', 'N/A')}"
                )

        # Save to file
        output_file = (
            Path("data")
            / f"citizen_ai_verified_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
