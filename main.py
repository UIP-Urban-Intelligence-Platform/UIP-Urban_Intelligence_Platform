...
def start_orchestrator_scheduler(interval_minutes: int = 60, workflow_config: str = "config/workflow.yaml"):
    """
    ...
    scheduler.start()
    logger.info(f"✅ Orchestrator scheduler started (interval: {interval_minutes} minutes)")
...
def main():
    ...
    if not args.no_orchestrator:
        logger.info(f"   ✓ Orchestrator Scheduler - Every {args.orchestrator_interval} minutes")
