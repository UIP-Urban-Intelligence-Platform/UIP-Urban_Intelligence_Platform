...
def download_model(model_name: str, output_dir: Path, force: bool = False) -> bool:
    ...
    if output_path.exists() and not force:
        ...
        if abs(file_size_mb - expected_size_mb) / expected_size_mb < 0.05:
            logger.info(
                f"✅ {model_name} already exists at {output_path} ({file_size_mb:.1f} MB)"
            )
            return True
        else:
...
def list_models() -> None:
    ...
    for name, info in YOLOX_MODELS.items():
        print(
            f"{name:<15} {info['size_mb']:<12.1f} {info['mAP']:<10.1f} {info['description']}"
        )
...
""",
    ...
    parser.add_argument(
        "--all", "-a", action="store_true", help="Download all available models"
    )
