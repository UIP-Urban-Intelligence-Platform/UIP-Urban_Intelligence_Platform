...
try:
    ...
    print("❌ MongoDB helper not available. Install pymongo: pip install pymongo>=4.6.0")
    ...
    def validate_entity(self, entity: Dict[str, Any]) -> bool:
        ...
        for key, value in entity.items():
            ...
            if not isinstance(value, dict):
                self.errors.append(f"Attribute '{key}' is not a dictionary (must be Property/GeoProperty/Relationship)")
                continue
            ...
            if attr_type == 'Property':
                ...
                if 'value' not in value:
                    ...
                    if not isinstance(geo_value, dict):
                        self.errors.append(f"GeoProperty '{key}' value must be a GeoJSON object")
                    elif 'type' not in geo_value or 'coordinates' not in geo_value:
                        self.errors.append(f"GeoProperty '{key}' must have 'type' and 'coordinates'")
...
class MongoDBDataVerifier:
    ...
    def sample_entity(self, collection_name: str) -> None:
        ...
        if entity:
            ...
            print(f"\nAttributes:")
            for key, value in entity.items():
                if key not in ['_id', 'id', 'type', '@context', '_insertedAt', '_updatedAt']:
