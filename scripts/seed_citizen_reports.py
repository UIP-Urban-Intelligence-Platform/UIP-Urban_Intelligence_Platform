...
def send_citizen_report(image_filename: str, report_num: int):
    ...
    try:
        ...
        if response.status_code == 202:
            ...
            print(
                f"✅ [{report_num:3d}/100] Report accepted: {result['reportId'][:8]}... | "
                f"{report_type:12s} | {user_id} | {image_filename[:30]}"
            )
            return True
        else:
            print(
                f"❌ [{report_num:3d}/100] Failed: HTTP {response.status_code} | {image_filename}"
            )
        ...
        print(
            f"❌ [{report_num:3d}/100] Connection Error: API not running at {API_URL}"
        )
        print("   Please start the Citizen Ingestion Agent first:")
        print(
            "   python -m uvicorn src.agents.ingestion.citizen_ingestion_agent:app --port 8001"
        )
        return False
...
def main():
    ...
    if success_count > 0:
        ...
        print("🔍 Next Steps:")
        print(
            "1. Check Stellio Context Broker: http://localhost:8080/ngsi-ld/v1/entities"
        )
        print(
            "2. Query citizen reports: http://localhost:8080/ngsi-ld/v1/entities?type=CitizenObservation"
        )
